...
based on random time interval * lastrandom * scale (log?)* - slow?
...
fast but low entropy?

-  and 4 possible trigger modes:

00: speedscale is divider
01: as below lastrandom*speedscale
10: maybe slow/fast doubles of above????
11:

discard code:

else {
// but we can also do this pulse/divider in main above trigger so runs fast!
//based on random time interval * lastrandom * scale (log?)* - slow?
