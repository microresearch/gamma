...
scaled by random (scaled by speedscale) ADDED
...
ISR (INT0_vect) // geiger interrupt
{
  ...
  // put divider and maybe other trigger modes here
    ...
    if (pulseonecounter>=pulsefresco){
      ...
      PORTD=0x30;
      if (trigmode==0) pulsefresco=(float)lessrandom*pgm_read_float(&floaty[speedscale]);
      else {
...
/*unsigned long speedlook[255] PROGMEM ={100, 102, 105, 108, 111, 114, 117, 121, 124, 127, 131, 135, 138, 142,
